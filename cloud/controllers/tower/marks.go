...
func GetMarks(ctx *gin.Context) {
	...
	rows, err := dao.DB().Raw("select i.* from images i,devices d where i.created_at>=? and i.created_at<? and i.device_id=d.id and d.tower_id=?", thisMonthBegin, nextMonthBegin, id).Rows()
	if err != nil {
		common.ResponseError(ctx, err)
		return
	}
