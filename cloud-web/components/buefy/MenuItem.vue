<template>
  <li :role="ariaRoleMenu">
    <component
      ...
      v-on="$listeners"
    >
      <b-icon v-if="icon" :icon="icon" :pack="iconPack" size="is-small" />
      <span v-if="label">{{ label }}</span>
      <slot v-else name="label" :expanded="newExpanded" :active="newActive" />
    </component>
    <!-- sub menu items -->
...
import config from '@/assets/js/buefy/config'
    ...
    reset(parent, menu) {
      ...
      items.forEach((item) => {
        if (item !== this) {
          ...
          if (
            !parent.$data._isMenu ||
            (parent.$data._isMenu && parent.accordion)
          ) {
